...
class AgentNN:
    ...
    def get_actionNN(self, obs):  # aggiornare sempre cosa vede l'agente!
        self.n_time += 1
        act = []
        #
        # SATISFY ORDERS:
    ...
    def learnFrequency(self, num_episode=1):
        ...
        for _ in tqdm(range(num_episode * self.time_limit)):
            ...
            action = self.get_actionNN(obs=obs)
            # print(action)
    ...
    def learn(self, iterations=10, learning_rate=0.3):
    ...
    def RunSimulation(self, iterations=10):
        for _ in tqdm(range(iterations)):
            obs = self.env.reset()
            self.actualDisposition = self.defineState(obs['actual_warehouse'])
