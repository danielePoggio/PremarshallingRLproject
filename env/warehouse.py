...
class Warehouse(gym.Env):
    ...
    def reset(self):
        self.disposition.disposition = np.zeros(
    ...
    def _get_obs(self):
        ...
        non_zeroes = np.where(self.disposition.disposition != 0, 1, 0)
        n_orders = 1
        high = min(3, self.n_cols * (self.n_rows-1) - np.sum(non_zeroes))
        if high == 0:
            n_parcels = 0
        # if high <= 1:
        #     print('Look at this!')
